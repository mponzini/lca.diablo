...
allpairs <- function(data, respvars, fixed, randomno1, randomno2, timevar,
                     time4mc, id, catvar) {
  # giving an id to the data in case we don't have id for the data
  data.table::setorderv(data, id)
  pairno <- 0
  ...
  for (l in 1:(nrespvars - 1)) {
    for (j in (l + 1):nrespvars) {
      ...
      # The parameter will be updated later for pair1 and pair2 -
      # we will pass more arguments into the functions
      if (length(randomno1) > 0) {
        pair_result <- pair1(
          rawdata = data, var1 = yl, var2 = yj, fixed = fixed,
          rand = randomno1, timevar = timevar, pairno = pairno, id = id,
          catvar = catvar
        )

        # I am not sure how to get the convstatus from the lmer package
        convstatus <- 1
      } else if (length(randomno2) > 0) {
        # pair2(var1 = yl, var2 = yj, fixed = fixed, rand1 = rl,
        #       rand2 = rj, timevar = timevar, pairno = pairno)
      }
      ...
      if (convstatus > 0) {
        ...
        # this V is variance-covariance matrix for all covairates,
        # we prob not going to use it
        # V <- c(V, vcov(pair_result$model))

        # Calculate Hessian Matrix
        Hessian_out <- hessian_pair(
  ...
  return(list(
    R_mean = R_mean, D_mean = D_mean, R = R, D = D, V = V, Y = Y, MU = MU,
    Fixedest = Fixedest, Hessian = H, Gradient = G,
    Fixedest_mean = fix_est_sum, pair_model = pair_model
  ))
}
