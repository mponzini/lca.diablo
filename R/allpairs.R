  ...
  for (l in 1:(nrespvars - 1)) {
    for (j in (l + 1):nrespvars) {
      ...
      if (length(randomno1) > 0) {
        ...
        convstatus <- 1  # I am not sure how to get convstatus from lmer package
      }
      else if (length(randomno2) > 0) {
        # pair2(var1=yl, var2=yj, fixed=fixed, rand1=rl, rand2=rj,
        #   timevar=timevar, pairno=pairno)
      }
      ...
      if (convstatus > 0) {
        ...
        # this V is variance-covariance matrix for all covariates,
