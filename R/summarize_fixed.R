...
summarize_fixed <- function(Fixedest, respvars, fixed, H, G) {
  ...
  parameter <- rep(
    rep(parameters, each = nrespvars - 1),
    choose(nrespvars, 2)
  )
  ...
  # estimate <- append(
  #   estimate,
  #   mean(fixed_[which(respvar == respvars[i] & parameter == parameters[j]), 1])
  # )

  # cov <- append(
  #   cov,
  #   mean(fixed_[which(respvar == respvars[i] & parameter == parameters[j]), 2])
  # )
