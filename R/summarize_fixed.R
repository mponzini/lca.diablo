...
summarize_fixed <- function(Fixedest, respvars, fixed, H, G) {
  ...
  for (i in 1:(nrespvars - 1)) {
    for (j in (i + 1):nrespvars) {
      respvar <- c(
        respvar,
        rep(c(respvars[i], respvars[j]), dim(Fixedest[[1]])[1] / 2)
      )
    }
  }
  ...
  # estimate <- append(estimate, mean(fixed_[which(respvar == respvars[i] &
  #   parameter == parameters[j]), 1]))

  # cov <- append(cov, mean(fixed_[which(respvar == respvars[i] &
  #   parameter == parameters[j]), 2]))
