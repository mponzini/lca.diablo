...
hessian_pair <- function(pair_result, H_, G_, V_, Y_, MU_, fixedest, pairn) {
  ...
  # residual from lmer will ignore the missing value, we have to adding them
  # back in to make sure the match of vector length
  ...
  resid <- c(resid(pair_result$model), fill_na)[order(as.numeric(names(
    c(resid(pair_result$model), fill_na)
  )))]
  ...
  for (i in 1:length(id_list)) {
    ...
    if (length(notmiss_index) > 0) {
      ...
      MU <- c(
        MU,
        list(as.numeric(
          apply(as.matrix(Xi), c(1, 2), as.numeric) %*% as.matrix(fixedest)
        ))
      )
    } else {
