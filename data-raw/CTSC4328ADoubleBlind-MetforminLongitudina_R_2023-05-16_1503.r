...
library(Hmisc)
#Read Data
data=read.csv('H:/Kyoungmi/Hagerman/Metformin_DIA-MS/Research/Data/CTSC4328ADoubleBlind-MetforminLongitudina_DATA_2023-05-16_1503.csv')
#Setting Labels
...
label(data$snap_adhd_combtot_sf)="ADHD Combined Total"
label(data$snap_odd_tot_sf)="ODD Total"
...
label(data$abc_comp_rev_sf)="ABC Composite Score-- REVISED"
label(data$ss_i_irrit_rev_sf)="Total - Subscale I (Irritability)-- REVISED"
label(data$ss_ii_leth_rev_sf)="Total - Subscale II (Lethargy)-- REVISED"
label(data$ss_iii_stereo_rev_sf)="Total - Subscale III (Stereotypy)--REVISED"
label(data$ss_iv_hyper_rev_sf)="Total - Subscale IV (Hyperactivity)--REVISED"
label(data$ss_v_inapp_rev_sf)="Total - Subscale V (Inappropriate Speech)--REVISED"
label(data$ss_vi_socavoid_rev_sf)="Total - Subscale VI (Social Avoidance)--REVISED"
...
label(data$adams_manic_sf)="Manic/Hyperactive Behavior Total"
label(data$adams_depress_sf)="Depressed Mood Total"
label(data$adams_avoid_sf)="Social Avoidance Total"
label(data$adams_anxious_sf)="General Anxiety Total"
label(data$adams_obsess_sf)="Obsessive/Compulsive Behavior Total"
...
label(data$vabs3_ac_ss_sf)="Adaptive Behavior Composite Standard Score"
label(data$vabs3_s_ss_sf)="Socialization Standard Score"
label(data$vabs3_dl_ss_sf)="Daily Living Skills Standard Score"
label(data$vabs3_c_ss_sf)="Communication Standard Score"
label(data$nar_ndw50_a)="Number of Different Words 50 - Analysis Set"
label(data$nar_ndw50_a_sf)="Number of Different Words 50 - Analysis Set"
...
label(data$tassone_cgg_sf)="CGG repeat number"
label(data$tassone_meth_sf)="% Methylation"
label(data$tassone_ar_sf)="Activation Ratio"
label(data$tassone_fmr1_sf)="FMR1 mRNA levels"
label(data$tassone_cyfip1_sf)="CYFIP1 mRNA levels"
label(data$tassone_mmp9_sf)="MMP9 normalized ratio"
label(data$tassone_cat_sf)="Molecular category"
#Setting Units
